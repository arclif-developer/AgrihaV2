...
export class ArcPaymentService {
  ...
  async findAll() {
    try{
    var arcplandata = await this.paymentModel.find({}).populate('architect_id').exec()
    
    return {status: 200, data:arcplandata,message:"plan data of architect"}
  } catch (error) {
    throw new NotFoundException(error)
  }
  }
  ...
  async findOne(id: ObjectId) {
    try {
    var arcplandata = await this.paymentModel.find({_id:id}).populate('architect_id').exec()
    
    return {status: 200, data:arcplandata,message:"payment data of architect"}
  } catch (error) {
    throw new NotFoundException(error)
  }
  }
  ...
  async findByArchitect(id: ObjectId) {
    try {
    var arcplandata = await this.paymentModel.find({architect_id:id}).populate('architect_id').exec()
  ...
  async update(id: ObjectId, updateArcPaymentDto: UpdateArcPaymentDto) {
    try {
      const user = await this.paymentModel
        .findByIdAndUpdate(id, { $set: updateArcPaymentDto })
      if (!user) {
  ...
  async remove(id: ObjectId) {
    return this.paymentModel.findByIdAndDelete(id).then(()=>{return "deleted"})
  }
}
